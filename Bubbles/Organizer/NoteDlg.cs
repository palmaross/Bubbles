    ...
    public partial class NoteDlg : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (edit) // note has been edited
            {
                ...
                tbLink.Location = StixMain.m_Notes.dummyLink.Location;
                tbTags.Location = StixMain.m_Notes.dummyTags.Location;
                ...
                {
                    if (Tags.Text.Trim() == "")
                        tbLink.Location = StixMain.m_Notes.dummyTags.Location;
                    else
                        tbLink.Location = StixMain.m_Notes.dummyLink.Location;
                }
            ...
            else // new note
            {
                ...
                StixMain.pNOTES.Remove((int)this.Tag);
                this.Tag = id;
                StixMain.pNOTES.Add(id, this);

                NoteGroupItem item = StixMain.m_Notes.cbGroups.SelectedItem as NoteGroupItem;
                if (item.ID == -1 || group.ID == item.ID) // "All groups" ot this group shown in the Notes window
                {
                    Panel newnote = StixMain.m_Notes.AddNote(
